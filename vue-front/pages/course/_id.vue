...
<div class="mt50 commentHtml"><div>
      ...
      <section class="">
          <section class="question-list lh-bj-list pr">
            <ul class="pr10">
              <li v-for="(comment,index) in info.items" v-bind:key="index">
                  <aside class="noter-pic">
        ...
        <div class="paging">
            ...
            <a
            :class="{undisable: !info.hasPrevious}"
            href="#"
            title="首页"
            ...
            href="#"
            title="前一页"
            @click.prevent="gotoPage(info.current-1)">&lt;</a>
            <a
            v-for="page in info.pages"
            :key="page"
            :class="{current: info.current == page, undisable: info.current == page}"
            :title="'第'+page+'页'"
            href="#"
            @click.prevent="gotoPage(page)">{{ page }}</a>
            <a
            :class="{undisable: !info.hasNext}"
            href="#"
            title="后一页"
            @click.prevent="gotoPage(info.current+1)">&gt;</a>
            <a
            ...
            href="#"
            title="末页"
            @click.prevent="gotoPage(info.pages)">末</a>
            <div class="clear"/>
        </div>
        <!-- 公共分页 结束 -->
      </div>
...
export default {
  ...
  methods:{
    initCourseInfo(){
      courseApi.getCourseInfo(courseId).then(response => {
    ...
    initComment(){
       comment.getCommentList(this.page, this.limit, this.courseId).then(response => {
         console.log(this.page+" "+this.limit+" "+this.courseId);
    ...
    gotoPage(page){
          comment.getCommentList(page, this.limit,this.courseId).then(response => {
              this.info = response.data.data
          })
      }
  }
